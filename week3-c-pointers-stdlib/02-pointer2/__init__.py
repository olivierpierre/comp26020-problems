...
def validate(sources_buf):
    if re.search(r"printf\(.*,\s*.*lm3.*\)", sources_buf):
        raise check50.Failure("Do not use lm3 in printf parameters")
    if re.search(r"printf\(.*,\s*.*lm2.*\)", sources_buf):
        raise check50.Failure("Do not use lm2 in printf parameters")

@check50.check(exists)
def validate2(sources_buf):
    if not re.search(r"printf\(.*,\s*lm1\s*.\s*next\s*->\s*next\s*->value\s*\)",
            sources_buf) and \
    not re.search(r"printf\(.*\(\s*\*\s*\(\s*\*\s*lm1\s*\.\s*next\s*\)\s*\.\s*next\s*\)\s*\.\s*value\s*\)", sources_buf) and \
    not re.search(r"printf\(.*\*\(\(\*\(lm1\.next\)\)\.next\)\)\.value", sources_buf):
